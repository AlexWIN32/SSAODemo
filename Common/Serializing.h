...
template<class TVar, class TypeCastFunc, class RandFunc>
TVar GetRandomFromRange(const std::string &FullString, TypeCastFunc TypeCastFunction, RandFunc RandFunction) throw (FormatException)
{
    auto vals = SerializerTools<StringParser>::PairFromString(FullString, '<', '>');

    TVar minVal = TypeCastFunction(vals.first.c_str()), maxVal = TypeCastFunction(vals.second.c_str());
...
public:
	...
	{ 			
        ...
        return GetRandomFromRange<int>(String, atoi, 
...
public:
	static float FromString(const std::string &String)
	{
        ...
        return GetRandomFromRange<float>(String, atof,
...
public:
    ...
    typedef Channels ValueType;
	static Channels FromString(const std::string &String)
	{
        return ParseValue<ColorParser<Parser>>(String, 
            [](const std::string &Str)
            {
                ArrayParser<Parser>::ValueType arr = ArrayParser<Parser>::FromString(Str);
                if(arr.size() != 4) 
                    throw FormatException("invalid color syntax for sring " + Str);
...
template <class T, class TChannels>
T Cast(const TChannels &ChannelsData)
{
    return T(ChannelsData.R, ChannelsData.G, ChannelsData.B, ChannelsData.A);
}
